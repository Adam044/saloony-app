...
self.addEventListener('push', (event) => {
  let payload = { title: 'Saloony', body: 'لديك إشعار جديد', url: '/admin_salon' };
  try {
    if (event.data) {
      // Support both JSON and plain text payloads
      try { payload = event.data.json(); }
      catch { payload = { title: 'Saloony', body: event.data.text(), url: '/admin_salon' }; }
    }
  } catch (e) {}

  const targetUrl = payload.url || '/admin_salon';
  const options = {
...
self.addEventListener('notificationclick', (event) => {
  ...
  const targetUrl = event.notification?.data?.url || '/admin_salon';
...
self.addEventListener('message', (event) => {
  ...
  if (msg.type === 'saloony-notify') {
    ...
    const targetUrl = msg.url || '/admin_salon';
    const options = {
      body,
